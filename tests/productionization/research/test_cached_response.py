...
def make_output(*, rating: str = "Hold") -> dict[str, object]:
    return {
        ...
        "investment_debate_state": {"bear_history": "", "bull_history": "", "count": 0, "current_response": "", "history": "", "judge_decision": ""},
        "investment_plan": "Fixture prose investment plan.",
        "market_report": "Fixture market evidence summary.",
        "messages": [["human", "NEW"], {"role": "assistant", "content": "Fixture research response.", "response_metadata": {"source": "test-only-cache"}}],
        "news_report": "Fixture news evidence summary.",
        "past_context": "",
        "risk_debate_state": {"aggressive_history": "", "conservative_history": "", "count": 0, "current_aggressive_response": "", "current_conservative_response": "", "current_neutral_response": "", "history": "", "judge_decision": "", "latest_speaker": "", "neutral_history": ""},
...
def make_capture_manifest(output: object, **updates: object) -> SourceManifest:
    fields: dict[str, object] = {
        "schema_version": "v1", "manifest_id": "cached-response-capture-v1",
        "source": "test-fixture", "source_locator": "fixture://research/cached-response/v1",
        "fetched_at": "2024-06-30T23:59:59Z", "event_time": "2024-06-30T23:59:59Z",
        "published_at": "2024-06-30T23:59:59Z", "available_at": "2024-06-30T23:59:59Z",
        "ingested_at": "2024-06-30T23:59:59Z", "checksum": checksum(output),
        "terms": "synthetic-test-fixture", "revision": 0,
    }
    fields.update(updates)
    return SourceManifest(**fields)  # type: ignore[arg-type]


def make_response_kwargs(*, bundle: object | None = None, context: object | None = None, output: dict[str, object] | None = None, **updates: object) -> dict[str, object]:
    bundle = bundle or build_fixture_bundle()
    output = output or make_output()
    fields: dict[str, object] = {
        "schema_version": CACHED_RESPONSE_SCHEMA_VERSION, "response_id": RESPONSE_ID,
        "bundle_id": bundle.bundle_id, "bundle_hash": bundle.bundle_hash,
        "knowledge_cutoff": bundle.knowledge_cutoff, "calendar_id": bundle.calendar.calendar_id,
        "replay_policy": bundle.replay_policy, "variant_id": "variant-research-adapter",
        "trade_date": "2024-06-30", "ticker": "NEW", "instrument_id": "inst-acme",
        "asset_type": "stock", "instrument_context": str(output["instrument_context"]),
        "graph_artifact_id": GRAPH_ARTIFACT_ID, "graph_artifact_hash": GRAPH_ARTIFACT_HASH,
        "model_artifact_id": MODEL_ARTIFACT_ID, "model_artifact_hash": MODEL_ARTIFACT_HASH,
        "runtime_manifest_id": RUNTIME_MANIFEST_ID, "runtime_manifest_hash": RUNTIME_MANIFEST_HASH,
        "capture_manifest": make_capture_manifest(output), "output": output,
    }
...
def make_selection(*, expected_response_hash: str | None = None, **updates: object) -> CachedGraphSelection:
    fields: dict[str, object] = {
...
def bound_kwargs(record: CachedGraphResponse, **updates: object) -> dict[str, object]:
    fields = {name: getattr(record, name) for name in (
        "bundle_id", "bundle_hash", "knowledge_cutoff", "calendar_id", "replay_policy",
...
def test_tuple_input_becomes_array_without_hooks() -> None:
    ...
    record = parse_cached_graph_response(build_cached_graph_response(**make_response_kwargs(output=output, capture_manifest=make_capture_manifest(output))))
    assert record.output["messages"] == [["human", "NEW"], ["assistant", "Fixture research response."]]


@pytest.mark.parametrize("mutation", [
    lambda p: p.update(schema_version="v2"), lambda p: p.update(output_hash=f"sha256:{'0' * 64}"),
...
@pytest.mark.parametrize("raw", [
    b"\xff", b'{"schema_version":"v1","schema_version":"v1"}',
    b'{"x":NaN}', b'{"x":Infinity}', b'{"x":-Infinity}', b'{"x":1e400}',
    b"{}\n", b'{ "x":1}', b'{"x": 1}', b'{"b":1,"a":2}',
])
def test_parser_rejects_bad_utf8_duplicates_nonfinite_overflow_and_noncanonical_raw(raw: bytes) -> None:
    with pytest.raises(CachedGraphResponseCorruptionError):
...
def test_parser_and_repository_require_exact_bytes_and_selection_types() -> None:
    ...
    with pytest.raises(CachedGraphResponseCorruptionError): parse_cached_graph_response(BytesSubclass(raw))
    with pytest.raises(CachedGraphResponseCorruptionError): repository.seal(BytesSubclass(raw))
    record = repository.seal(raw)
    class SelectionSubclass(CachedGraphSelection):
        def model_dump(self, *args: object, **kwargs: object) -> object: raise AssertionError((args, kwargs))
    hostile = SelectionSubclass.model_construct(**make_selection(expected_response_hash=record.response_hash).model_dump(mode="python"))
    with pytest.raises(CachedGraphResponseMismatchError): repository.get_bound(hostile, **bound_kwargs(record))
...
def test_repository_seal_is_idempotent_and_returns_defensive_reparses() -> None:
    ...
    assert repository.get_bound(selection, **bound_kwargs(first)) == first
...
def test_repository_rejects_same_id_with_different_bytes() -> None:
    ...
    with pytest.raises(CachedGraphResponseConflictError):
        repository.seal(build_cached_graph_response(**make_response_kwargs(output=make_output(rating="Sell"), capture_manifest=make_capture_manifest(make_output(rating="Sell")))))


def test_repository_missing_and_tampered_storage_fail_typed() -> None:
    repository = CachedGraphResponseRepository()
    with pytest.raises(CachedGraphResponseUnavailableError):
        repository.get_bound(make_selection(), **bound_kwargs(parse_cached_graph_response(FIXTURE.read_bytes().removesuffix(b"\n"))))
    ...
    with pytest.raises(CachedGraphResponseCorruptionError):
        repository.get_bound(make_selection(expected_response_hash=record.response_hash), **bound_kwargs(record))


@pytest.mark.parametrize(("field", "replacement"), [
    ("bundle_id", "bundle-other"), ("bundle_hash", f"sha256:{'4' * 64}"),
    ("knowledge_cutoff", "2024-06-30T23:59:58Z"), ("calendar_id", "calendar-other"),
    ("replay_policy", BundleReplayPolicy.AVAILABILITY), ("variant_id", "variant-other"),
    ("trade_date", "2024-06-29"), ("ticker", "OLD"), ("instrument_id", "instrument-other"),
    ("asset_type", "crypto"), ("instrument_context", "different sealed context"),
])
def test_get_bound_denies_every_bundle_context_instrument_binding(field: str, replacement: object) -> None:
    repository = CachedGraphResponseRepository()
    record = repository.seal(FIXTURE.read_bytes().removesuffix(b"\n"))
    with pytest.raises((CachedGraphResponseMismatchError, CachedGraphResponseUnavailableError)):
        repository.get_bound(make_selection(expected_response_hash=record.response_hash), **bound_kwargs(record, **{field: replacement}))


@pytest.mark.parametrize(("field", "replacement"), [
    ("response_id", "response-other"), ("expected_response_hash", f"sha256:{'4' * 64}"),
    ("graph_artifact_id", "graph-other"), ("graph_artifact_hash", f"sha256:{'4' * 64}"),
    ("model_artifact_id", "model-other"), ("model_artifact_hash", f"sha256:{'4' * 64}"),
    ("runtime_manifest_id", "runtime-other"), ("runtime_manifest_hash", f"sha256:{'4' * 64}"),
])
def test_get_bound_denies_every_exact_selection_binding(field: str, replacement: str) -> None:
    repository = CachedGraphResponseRepository()
    record = repository.seal(FIXTURE.read_bytes().removesuffix(b"\n"))
    selection = make_selection(expected_response_hash=record.response_hash, **{field: replacement})
    error = CachedGraphResponseUnavailableError if field == "response_id" else CachedGraphResponseMismatchError
    with pytest.raises(error): repository.get_bound(selection, **bound_kwargs(record))


@pytest.mark.parametrize(("policy", "available_delta", "ingested_delta", "allowed"), [
    (BundleReplayPolicy.AVAILABILITY, -1, 1, True), (BundleReplayPolicy.AVAILABILITY, 0, 1, True),
    (BundleReplayPolicy.AVAILABILITY, 1, 1, False), (BundleReplayPolicy.ARCHIVE_REALISTIC, -1, -1, True),
    (BundleReplayPolicy.ARCHIVE_REALISTIC, 0, 0, True), (BundleReplayPolicy.ARCHIVE_REALISTIC, -1, 1, False),
])
def test_capture_cutoffs_follow_availability_and_archive_policy(policy, available_delta, ingested_delta, allowed):
    bundle = build_fixture_bundle()
    output = make_output()
    cutoff = bundle.knowledge_cutoff
    manifest = make_capture_manifest(output, published_at=None, available_at=cutoff + timedelta(seconds=available_delta), fetched_at=cutoff + timedelta(seconds=max(available_delta, 0)), ingested_at=cutoff + timedelta(seconds=ingested_delta))
    kwargs = make_response_kwargs(bundle=bundle, output=output, replay_policy=policy, capture_manifest=manifest)
    if allowed:
        assert parse_cached_graph_response(build_cached_graph_response(**kwargs)).replay_policy is policy
    else:
        with pytest.raises(CachedGraphResponseUnavailableError): build_cached_graph_response(**kwargs)
...
def nested(depth: int) -> dict[str, object]:
    value: object = "leaf"
    for index in range(depth): value = {f"n{index}": value}
...
def test_limits_reject_record_bytes_depth_nodes_and_string_size() -> None:
    ...
    with pytest.raises(CachedGraphResponseCorruptionError, match="size"):
        build_cached_graph_response(**make_response_kwargs(output=output, capture_manifest=make_capture_manifest(output)))
    output = make_output(); output["messages"].append(nested(MAX_CACHED_RESPONSE_DEPTH + 1))
    with pytest.raises(CachedGraphResponseCorruptionError, match="depth"):
        build_cached_graph_response(**make_response_kwargs(output=output, capture_manifest=make_capture_manifest(output)))
    output = make_output(); output["messages"].extend("x" for _ in range(MAX_CACHED_RESPONSE_NODES))
    with pytest.raises(CachedGraphResponseCorruptionError, match="node"):
        build_cached_graph_response(**make_response_kwargs(output=output, capture_manifest=make_capture_manifest(output)))
    output = make_output(); output["market_report"] = "é" * (MAX_CACHED_RESPONSE_STRING_BYTES // 2 + 1)
...
@pytest.mark.parametrize("placement", ["opaque", "callable", "dict-subclass", "list-subclass", "string-subclass", "non-string-key"])
def test_builder_rejects_opaque_custom_callback_and_non_string_key_without_hooks(placement: str) -> None:
    calls: list[str] = []
    class Opaque:
    ...
    output = make_output()
    if placement == "opaque": output["messages"].append(Opaque())
    elif placement == "callable": output["messages"].append(lambda: None)
    elif placement == "dict-subclass": output["messages"].append(DictSubclass(role="assistant", content="x"))
    elif placement == "list-subclass": output["messages"] = ListSubclass(output["messages"])
    elif placement == "string-subclass": output["market_report"] = StringSubclass("x")
    else: output["messages"].append({1: "not-a-string-key"})
    with pytest.raises(CachedGraphResponseCorruptionError):
