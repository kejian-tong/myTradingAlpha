...
def test_calendar_preserves_dst_transition_and_actual_early_close() -> None:
    ...
    assert early_close.close_at.astimezone(eastern).hour == 13
...
def test_calendar_ranges_are_inclusive_and_next_session_is_strictly_later() -> None:
    ...
    assert calendar.next_session(date(2024, 7, 4)).session_date == date(2024, 11, 27)
...
def test_as_of_cutoff_is_inclusive_and_selects_highest_available_revision() -> None:
    calendar = _calendar()
    session = calendar.session("2024-11-27")
    revisions = tuple(_bar(calendar, revision=revision) for revision in range(3))
    ...
    assert selected.manifest.available_at == session.close_at + timedelta(minutes=2)
...
def test_as_of_uses_every_source_and_adjustment_selector_without_fallback() -> None:
    calendar = _calendar()
    session = calendar.session("2024-11-27")
    ...
    assert selected.manifest.source == "alternate-bars"
...
def test_as_of_distinguishes_future_stale_and_missing() -> None:
    ...
    repository = _repository(calendar, (prior, future_exact))
    ...
    stale_repository = _repository(calendar, (prior,))
