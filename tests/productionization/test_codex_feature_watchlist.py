...
@pytest.mark.parametrize(
    ("accepted_clause", "hostile_clause"),
    (
        ("remain higher-precedence overrides", "are not higher-precedence overrides"),
        ("does not retroactively change running sessions", "retroactively changes running sessions"),
    ),
)
def test_checker_rejects_memory_watchlist_contradictory_policy_reason(
    tmp_path: Path, accepted_clause: str, hostile_clause: str
) -> None:
    checker = _checker()
    fixture = _copy_harness_fixture(tmp_path)
    path = fixture / "docs/productionization/CODEX_FEATURE_WATCHLIST.md"
    ...
    hostile_row = _GOOD_MEMORY_WATCHLIST_ROW.replace(accepted_clause, hostile_clause)
    path.write_text(_replace_memory_watchlist_row(original, hostile_row), encoding="utf-8")
    errors = checker.configuration_errors(fixture)
    assert any("memory" in error.lower() and "watch" in error.lower() for error in errors), errors
