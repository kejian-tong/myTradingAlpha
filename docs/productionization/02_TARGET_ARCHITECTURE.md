...
The MVP supports long-only, unlevered liquid US equities/ETFs from a small allowlist. A run makes a close decision and may execute no earlier than the next eligible session. FND-01 through FND-04, PIT-01 through PIT-06, SIG-01 and SIG-02 are implemented at their approved contract scope; SIG-03 is implemented in active PR #87 but remains pending review, CI, and merge. SIG-03 adds only deterministic shadow QuantSignal calculation; it does not add portfolio authority, orders, PAPER or live behavior. SIG-04 and subsequent behavior remain planned. Use the [current implementation index](README.md#current-implementation-and-evidence-index) and actual GitHub state rather than interpreting the diagram as merged functionality.
## System overview
