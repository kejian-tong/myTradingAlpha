...
| Research adapter | `research/test_adapter.py` | Exact repository/bundle ID/hash/cutoff/calendar/RunContext binding; explicitly injected offline runtime once; current initial/final/five-tier shape; pending/current-provider/model/persistence/checkpoint/file/socket/clock denied; runtime unavailable/type/output and authority fields fail closed; ordinary graph remains default | `pytest -q tests/productionization/research/test_adapter.py` |
