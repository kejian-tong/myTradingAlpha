...
Historical replay reads an immutable `EvidenceBundle` and, for a model-bearing research variant, a separately sealed cached-response record selected by exact ID/hash and bound to that bundle/context. Responses are not embedded in EvidenceBundle v1. Every component-scoped egress field is false. Both observations and cached responses require `available_at <= knowledge_cutoff`; archive-realistic replay additionally requires `ingested_at <= knowledge_cutoff`. Availability-only backfilled research is not archive-realistic evidence and never permits backdating a newly generated response. Preserve publication time, event time, validity intervals, revisions, and replay policy. The [approved SIG-01 amendment](phases/02-evidence-agent-boundary/SIG_01_AMENDMENT_PROPOSAL.md) and [capture handoff](03_CONTRACTS_AND_SCHEMAS.md#closed-response-capture-and-replay-handoff) define this boundary. Forward paper may enable approved data capture/model-provider/PAPER egress while research-tool and live-broker egress remain false.
...
contract scope. SIG-03 is implemented in active PR #87 from verified base
`49d5980b640638ed687b6c7771f5f28367072c9a`, pending independent review, required CI, and the Master
merge gate; it is not yet merged. SIG-04 and later roadmap slices remain planned. SIG-02 merged as PR #45 /
`376c9c044722ee37f3fa36691b576420e3b6253d` and adds the
